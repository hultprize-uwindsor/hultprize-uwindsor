...
import { useState, useEffect, useCallback, type CSSProperties, forwardRef, useImperativeHandle } from "react";
...
export interface PageFlipLeaf {
    ...
    frontAlt?: string;
    backAlt?: string;
    frontSpread?: "left" | "right";
    backSpread?: "left" | "right";
    frontTitle?: string;
    backTitle?: string;
...
export interface ThreeDImagePageflipProps {
    ...
    pageWidth?: number;
    /** Keep the first and last leaves pinned so the book stays open. */
    keepOpen?: boolean;
...
export const ThreeDImagePageflip = forwardRef<ThreeDImagePageflipHandle, ThreeDImagePageflipProps>(({
    ...
    const minTurned = keepOpen && totalLeaves > 1 ? 1 : 0;
    const maxTurned = keepOpen && totalLeaves > 1 ? totalLeaves - 1 : totalLeaves;
    const currentTurned = Math.max(minTurned, Math.min(
    ...
    const setTurned = useCallback((newCount: number) => {
        const clamped = Math.max(minTurned, Math.min(newCount, maxTurned));
    ...
    }, [controlledTurnedIndex, totalLeaves, minTurned, maxTurned, onPageChange]);

    const flipNext = useCallback(() => {
        if (currentTurned < maxTurned) {
            setTurned(currentTurned + 1);
        }
    }, [currentTurned, maxTurned, setTurned]);

    const flipPrev = useCallback(() => {
        if (currentTurned > minTurned) {
            setTurned(currentTurned - 1);
        }
    }, [currentTurned, minTurned, setTurned]);
    ...
    useEffect(() => {
        ...
        const timer = setInterval(() => {
            setTurned(currentTurned >= maxTurned ? minTurned : currentTurned + 1);
        }, autoplayInterval);
        return () => clearInterval(timer);
    }, [autoplay, autoplayInterval, pauseOnHover, isHovered, totalLeaves, currentTurned, minTurned, maxTurned, setTurned]);
        ...
        >
                ...
                >
                    ...
                    {showSpineBinding && (
                        <div
                            ...
                            style={{
                                boxShadow: shadowIntensity === 0 ? "none" : undefined,
                    ...
                    <div
                        ...
                        style={{
                            opacity: shadowIntensity === 0 ? 0 : (isOpen ? 0.7 : 0.4),
                    ...
                    {pages.map((leaf, index) => {
                        ...
                        const isPinned = keepOpen && totalLeaves > 1 && (index === 0 || index === totalLeaves - 1);
                        const isCanPeek = interactive && !isPinned && index === currentTurned;
                        ...
                        if (!isTurned && isPeeking && isCanPeek) {
                        ...
                        return (
                            <div
                                ...
                                className={cn(
                                    ...
                                    isPinned ? "cursor-default" : interactive ? "cursor-pointer" : "pointer-events-none"
                                )}
                                style={{
                                    transformStyle: "preserve-3d",
                                    transition: `transform ${duration}s ${easing}`,
                            ...
                            >
                                ...
                                >
                                    <img
                                        src={leaf.frontImage}
                                        alt={leaf.frontAlt ?? leaf.frontTitle ?? `Page ${index * 2 + 1}`}
                                        className="w-full h-full object-cover pointer-events-none select-none"
                                        style={leaf.frontSpread ? {
                                            width: "200%",
                                            maxWidth: "none",
                                            objectFit: "cover",
                                            transform: leaf.frontSpread === "right" ? "translateX(-50%)" : undefined,
                                        } : undefined}
                                        data-spread-half={leaf.frontSpread}
                                        loading="lazy"
                                    />
                                    ...
                                    {(showPageNumbers || leaf.frontTitle || leaf.frontSubtitle || leaf.frontBadge) && (<div className="absolute inset-x-0 bottom-0 p-3.5 bg-gradient-to-t from-black/85 via-black/35 to-transparent text-white pointer-events-none">
                                ...
                                >
                                    <img
                                        src={leaf.backImage}
                                        alt={leaf.backAlt ?? leaf.backTitle ?? `Page ${index * 2 + 2}`}
                                        className="w-full h-full object-cover pointer-events-none select-none"
                                        style={leaf.backSpread ? {
                                            width: "200%",
                                            maxWidth: "none",
                                            objectFit: "cover",
                                            transform: leaf.backSpread === "right" ? "translateX(-50%)" : undefined,
                                        } : undefined}
                                        data-spread-half={leaf.backSpread}
                                        loading="lazy"
                                    />
                                    ...
                                    {/* Bottom Vignette & Metadata */}
                                    {(showPageNumbers || leaf.backTitle || leaf.backSubtitle || leaf.backBadge) && (<div className="absolute inset-x-0 bottom-0 p-3.5 bg-gradient-to-t from-black/85 via-black/35 to-transparent text-white pointer-events-none">
                                        <div className="flex items-center justify-between gap-2 mb-1">
                                            {leaf.backBadge && (
                                                <span
                                                    className="px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider backdrop-blur-md border border-white/20"
            ...
            {showControls && (
                <div className="flex items-center justify-center gap-3 mt-4 select-none">
                    <button
                        onClick={flipPrev}
                        disabled={currentTurned === minTurned}
                    ...
                    <button
                        onClick={flipNext}
                        disabled={currentTurned === maxTurned}
